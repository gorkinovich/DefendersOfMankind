...
void CServer::createScene ()
{
	...
	// Activar / desactivar la detección de colisiones entre grupos de colisión.
	// En realidad solo hace falta indicar los grupos que NO colisionan entre sí (por defecto todos colisionan con todos)
...
bool CServer::checkHardColision(IPhysicObj* physicObj, const Vector3& center, const Vector3& dim)
{
	...
	// Creamos el AABB con el que comparar la colisión (representa la caja de la muerte)
	NxBounds3 deathBox;
	NxVec3 min, max;
	min.x = center.x - (dim.x/2);	max.x = center.x + (dim.x/2);	// Calculamos adecuadamente la posición del AABB
	min.y = center.y - (dim.y/2);	max.y = center.y + (dim.y/2);	// con respecto al centro y las dimensiones pasados
	min.z = center.z - (dim.z/2);	max.z = center.z + (dim.z/2);	// como parámetros

	deathBox.set(min, max);		//deathBox.set(NxVec3(200,-100,-1000), NxVec3(400,200,0));

	// Para cada actor del objeto físico de la entidad...

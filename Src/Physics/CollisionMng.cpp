...
void  CCollisionMng::onTrigger (const TActorInfo &triggerInfo, const TActorInfo &otherInfo, NxTriggerFlag status)
{
	...
	PhysicEntity *triggerComponent = reinterpret_cast<PhysicEntity*>(triggerInfo.pPhysicObj->userData);
	Core::Entity *triggerEntity = triggerComponent->getOwner();

	// Obtener el componente y la entidad asociados al otro actor
	PhysicEntity *otherComponent = reinterpret_cast<PhysicEntity*>(otherInfo.pPhysicObj->userData);
	Core::Entity *otherEntity = otherComponent->getOwner();

...
namespace AI 
{
	...
	*/
	void CKinematicSeek::move(unsigned int msecs, DynamicMovement& currentProperties)
	{
		...
		currentProperties.linearSpeed = _target - _entity->getComponentAs<Logic::Position>("Position")->getPosition();
	...
	*/
	void CKinematicAlignToSpeed::move(unsigned int msecs, DynamicMovement& currentProperties)
	{
		...
		currentProperties.angularSpeed = (atan2(-currentProperties.linearSpeed.x, -currentProperties.linearSpeed.z) -
			_entity->getComponentAs<Logic::Position>("Position")->getYaw());
		...
		if (abs(currentProperties.angularSpeed) > _maxAngularSpeed) {
			currentProperties.angularSpeed = Ogre::Math::Sign((Ogre::Real)currentProperties.angularSpeed) * _maxAngularSpeed;
		}
	}
} //namespace AI
...
public:
	...
	*/
	void addEdge(int node1, int node2, float cost);
	/**
	...
	*/
	const std::list<int> &getNeighbours(int idNode);
	/**
	...
	*/
	...
	@return Coste.
	*/
	float getEdgeCost(int node1, int node2) { return _cost.getElement(node1, node2); };
...
private:
	...
	*/
	std::map<int, std::list<int>*> _adjacency;
	/**
	...
	*/
	void setAdjacent(int orig, int dest);
	/**
